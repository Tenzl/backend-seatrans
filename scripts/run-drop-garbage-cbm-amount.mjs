...
const sqlPath = join(root, 'scripts/drop-garbage-cbm-amount.sql');
  ...
  const before = await client.query(`
    SELECT EXISTS (
      ...
      WHERE table_name = 'shipping_agency_inquiries'
        AND column_name = 'garbage_cbm_amount'
    ) AS exists
  `);
    ...
    JSON.stringify(
      {
        ...
        columnExistsAfter: Boolean(after.rows[0]?.exists),
      },
      null,
      2,
    ),
  );
  console.log('[migrate] garbage_cbm_amount dropped');
} finally {
  await client.end();
}
