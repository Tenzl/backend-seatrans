...
} from './lib/booking-partner-dedupe-remaining.mjs';
...
const sqlPath = join(
  projectRoot,
  'scripts/migrations/2026-08-20_booking_partner_dedupe_remaining.sql',
);
const lockName = 'seatrans:booking-partner-dedupe-remaining:2026-08-20';
const fieldMap = {
  name: 'name',
  country: 'country',
  city: 'city',
  ...
  const result = await client.query(
    `SELECT
      (SELECT count(*)::int FROM booking_records WHERE deleted_at IS NULL
        AND payload->>'clientPartyId' = ANY($1::text[])) AS booking_client,
      (SELECT count(*)::int FROM bill_of_lading_records WHERE deleted_at IS NULL
        AND (payload->>'shipperPartyId' = ANY($1::text[])
          OR payload->>'consigneePartyId' = ANY($1::text[])
          OR payload->>'notifyPartyId' = ANY($1::text[]))) AS bill_parties,
      (SELECT count(*)::int FROM arrival_notice_records WHERE deleted_at IS NULL
        AND (payload->>'agentPartyId' = ANY($1::text[])
          OR payload->>'shipperPartyId' = ANY($1::text[])
          OR payload->>'consigneePartyId' = ANY($1::text[])
          OR payload->>'notifyPartyId' = ANY($1::text[]))) AS arrival_parties,
      (SELECT count(*)::int FROM delivery_order_records WHERE deleted_at IS NULL
        AND (payload->>'consigneePartyId' = ANY($1::text[])
          OR payload->>'notifyPartyId' = ANY($1::text[]))) AS delivery_parties`,
    [ids],
  );
...
function validateTargets(targets) {
  ...
  const ids = targets.partners.map(({ id }) => Number(id));
  if (
    new Set(ids).size !== allIds.length ||
    allIds.some((id) => !ids.includes(id))
  ) {
  ...
  if (
    targets.partners.some(
      ({ contacts }) => contacts != null && !Array.isArray(contacts),
    )
  ) {
    throw new Error('Every target contacts value must be a JSON array or null');
  }
  return {
...
function expectedTypes(targets) {
  const byId = new Map();
  for (const row of targets.additionTypes) {
    byId.set(row.partner_id, [
      ...(byId.get(row.partner_id) ?? []),
      row.addition_type,
    ]);
  }
  return new Map(
...
async function activeDuplicateGroups(client) {
  const result = await client.query(
    `SELECT id, name FROM booking_partners
  ...
  return [...groups.entries()]
    ...
    .map(([normalizedName, ids]) => ({ normalizedName, ids }));
}

async function verifyPostflight(client, beforeTargets) {
  const { keepIds, duplicateIds } = validatePairConfiguration();
  const result = await client.query(
    `SELECT to_jsonb(partner) AS row FROM booking_partners AS partner
  ...
  if (duplicateIds.some((id) => byId.has(id))) {
    throw new Error('One or more remaining duplicate partners still exist');
  }
  if (keepIds.some((id) => !byId.has(id))) {
    throw new Error('One or more remaining keeper partners are missing');
  }
  for (const [keepIdText, resolved] of Object.entries(RESOLVED_VALUES)) {
    const keepId = Number(keepIdText);
    const row = byId.get(keepId);
    for (const [property, expected] of Object.entries(resolved)) {
      const column = fieldMap[property];
      if (!column || row[column] !== expected) {
        throw new Error(`Keeper ${keepId} resolution mismatch: ${property}`);
      }
    }
  }
  const actualTypesResult = await client.query(
    `SELECT partner_id, addition_type::text FROM booking_partner_addition_types
      WHERE partner_id = ANY($1::int[])
      ORDER BY partner_id, addition_type::text`,
    [keepIds],
  );
  const actualTypes = new Map();
  for (const row of actualTypesResult.rows) {
    actualTypes.set(row.partner_id, [
      ...(actualTypes.get(row.partner_id) ?? []),
      row.addition_type,
    ]);
  }
  for (const [keepId, expected] of expectedTypes(beforeTargets)) {
    if ((actualTypes.get(keepId) ?? []).join(',') !== expected.join(',')) {
      throw new Error(`Keeper ${keepId} addition types mismatch`);
    }
  }
  const duplicateGroups = await activeDuplicateGroups(client);
  if (duplicateGroups.length !== 0) {
    throw new Error(
      `Active normalized partner duplicates remain: ${JSON.stringify(duplicateGroups)}`,
    );
  }
  return {
    keepers: keepIds.length,
    deletedDuplicates: duplicateIds.length,
    typeRows: actualTypesResult.rows.length,
    activeNormalizedDuplicateGroups: 0,
  };
}
...
async function main() {
  ...
  try {
    const ledger = await ledgerRow(client);
    if (ledger?.status === 'SUCCEEDED') {
      if (ledger.script_checksum !== sqlChecksum) {
        throw new Error('Applied remaining migration SQL checksum changed');
      }
      if (!ledger.backup_reference || !existsSync(ledger.backup_reference)) {
        throw new Error('Applied remaining migration backup is missing');
      }
      ...
      if (
        ...
        throw new Error(
          'Applied remaining migration backup checksum is invalid',
        );
      }
      const verified = await verifyPostflight(client, backup.targets);
      const usage = await documentUsage(client, duplicateIds);
      if (usage.total !== 0) {
        throw new Error('Deleted duplicate IDs are referenced by documents');
      }
      console.log(
        JSON.stringify({
          mode: 'postflight',
    ...
    if (args.simulate || args.apply) {
      const lock = await client.query(
        'SELECT pg_try_advisory_lock(hashtext($1)) AS acquired',
        [lockName],
      );
      lockAcquired = lock.rows[0]?.acquired === true;
      if (!lockAcquired) throw new Error('Remaining dedupe is already running');
    ...
    if (!args.apply && !args.simulate) {
      ...
      try {
        ...
        const usage = await documentUsage(client, duplicateIds);
        if (usage.total !== 0) {
          throw new Error('Remaining duplicate partners are referenced');
        }
    ...
    if (args.simulate) {
      ...
      try {
        const targets = await loadTargets(client, true);
        const validation = validateTargets(targets);
        const usage = await documentUsage(client, duplicateIds);
        if (usage.total !== 0) {
          throw new Error('Remaining duplicate partners are referenced');
    ...
    const targetChecksum = checksumTargets(targets);
    const backup = {
      format: 'seatrans-booking-partner-dedupe-backup-v2',
    ...
    await client.query('BEGIN ISOLATION LEVEL SERIALIZABLE');
    try {
      const lockedTargets = await loadTargets(client, true);
      if (checksumTargets(lockedTargets) !== targetChecksum) {
        throw new Error('Remaining target partners changed after backup');
      }
      const lockedUsage = await documentUsage(client, duplicateIds);
      if (lockedUsage.total !== 0) {
        throw new Error('Remaining duplicates gained document references');
      }
      await client.query(
