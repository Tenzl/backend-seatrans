 ...
 * primary key draws from a single shared Postgres sequence
 * (`inquiry_global_id_seq`, installed by the controlled runtime-schema
 * migration) so inquiry ids stay globally unique across all service tables.
 * Documents, notifications and audit logs reference an inquiry by bare id, so
 * per-table sequences would allow collisions.
 */
